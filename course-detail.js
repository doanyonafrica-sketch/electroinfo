...
window.openSession = function(seqIndex, sessionIndex) {
    ...
    // Rediriger vers la page de détails de la séance (slug SEO ou fallback ID)
    window.location.href = currentCourse.slug
        ? `/seance/${currentCourse.slug}/seq-${seqIndex + 1}/seance-${sessionIndex + 1}`
        : `/session-detail?courseId=${currentCourse.id}&seqIndex=${seqIndex}&sessionIndex=${sessionIndex}`;
};
