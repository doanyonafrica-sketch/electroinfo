...
articleForm?.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        const articleData = {
            ...
            contentType: editorMode,   // ✅ FIX 1: type éditeur sauvegardé (html ou quill)
...
window.editArticle = async function(articleId) {
    try {
        ...
        // ✅ FIX 2: Détection automatique du bon éditeur
        // Si le contenu a des style= inline, on force le mode HTML Brut
        const savedContentType = article.contentType || 'quill';
        const hasInlineStyles = article.content && article.content.includes('style=');
        const shouldUseHTML = savedContentType === 'html' || hasInlineStyles;

        if (shouldUseHTML) {
            // Forcer le mode HTML Brut pour préserver les styles inline
            window.articleEditorMode = 'html';
            // Activer automatiquement l'onglet "HTML Brut"
            const allButtons = Array.from(document.querySelectorAll('button'));
            const htmlTabBtn = allButtons.find(btn =>
                btn.dataset.mode === 'html' ||
                btn.classList.contains('tab-html') ||
                (btn.textContent.includes('HTML') && btn.textContent.includes('Brut'))
            );
            if (htmlTabBtn) htmlTabBtn.click();
            // Charger dans CodeMirror avec délai pour laisser l'onglet s'activer
            setTimeout(() => {
            ...
            // Mode Quill standard (sans styles inline)
            window.articleEditorMode = 'quill';
