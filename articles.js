...
// ============================================
// LANGUE — clé 'electroinfo_lang' : 'ewe' | 'fr' | 'en'
// Dans Firestore : clé 'ee' pour l'Éwé
// ============================================
let _popularArticlesCache = [];
...
function getTranslated(article, field) {
    ...
    const key = lang === 'ewe' ? 'ee' : lang;
    return article?.translations?.[key]?.[field] || article[field] || '';
}
function getLangText(fr, en, ewe) {
    const lang = getCurrentLang();
...
function saveArticlesToCache(articles, isComplete = false) {
    try {
        // On ne garde que les champs légers — on exclut content (HTML lourd) et translations.content
        const lightweight = articles.map(a => {
            const slim = {
                id: a.id, title: a.title, summary: a.summary, category: a.category,
                imageUrl: a.imageUrl, slug: a.slug, views: a.views,
                commentsCount: a.commentsCount, status: a.status,
                createdAt: a.createdAt, publishedAt: a.publishedAt
            };
            // Garder les traductions SANS le champ content (trop lourd)
            if (a.translations) {
                slim.translations = {};
                for (const [lang, t] of Object.entries(a.translations)) {
                    slim.translations[lang] = {
        ...
        const data = { articles: lightweight, timestamp: Date.now(), version: '2.0', isComplete };
        localStorage.setItem(CACHE_KEY, JSON.stringify(data));
        console.log(`${lightweight.length} articles sauvegardés en cache`);
    } catch (e) {
        // Si localStorage est plein, on vide l'ancien cache et on réessaie une fois
        console.warn('Cache plein, nettoyage et réessai...', e.name);
        try {
            localStorage.removeItem(CACHE_KEY);
            localStorage.removeItem(POPULAR_CACHE_KEY);
            localStorage.removeItem('electroinfo_admin_cache_articles');
            const data = { articles: articles.slice(0, 10).map(a => ({
                id: a.id, title: a.title, summary: a.summary, category: a.category,
                imageUrl: a.imageUrl, slug: a.slug, views: a.views,
                commentsCount: a.commentsCount, status: a.status, createdAt: a.createdAt
            })), timestamp: Date.now(), version: '2.0', isComplete: false };
            localStorage.setItem(CACHE_KEY, JSON.stringify(data));
        } catch (e2) {
            console.warn('Cache désactivé (localStorage saturé)');
        }
    }
}
...
function createFeaturedCard(article) {
    ...
    const readLabel     = getLangText('Lire', 'Read', 'Xlee');
    const viewsLabel    = getLangText('vues', 'views', 'kpɔkpɔwo');
    const featuredLabel = getLangText('À la une', 'Featured', 'Ŋgɔ la');
    const hasTrans = article.translations && Object.keys(article.translations).length > 0;
    ...
    return `<article class="article-card article-card--featured" onclick="window.location.href='${articleUrl}'">
        <div class="article-card__image-wrap">
            ...
            ${hasTrans ? '<span style="position:absolute;top:8px;right:8px;background:rgba(30,64,175,0.85);color:white;border-radius:6px;padding:3px 8px;font-size:11px;font-weight:700;">🌍</span>' : ''}
        </div>
...
function createArticleCard(article) {
    ...
    return `<article class="article-card" onclick="window.location.href='${articleUrl}'">
        <div class="article-card__image-wrap">
            ...
            ${isOffline ? '<span class="article-card__offline-badge"><i class="fas fa-database"></i> Cache</span>' : ''}
            ${hasTrans ? '<span style="position:absolute;top:6px;right:6px;background:rgba(30,64,175,0.85);color:white;border-radius:5px;padding:2px 6px;font-size:10px;font-weight:700;">🌍</span>' : ''}
        </div>
        <div class="article-content">
            <div class="article-meta">
                <span class="badge badge-${categoryClass}">${escapeHtml(category)}</span>
...
function displayPopularArticles(articles) {
    _popularArticlesCache = articles || [];
...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    // 🌍 Re-rendre quand l'utilisateur change de langue
    document.querySelectorAll('.lang-option').forEach(btn => {
        btn.addEventListener('click', () => {
            setTimeout(() => {
                if (allArticles.length > 0) displayArticles(true);
                if (_popularArticlesCache.length > 0) displayPopularArticles(_popularArticlesCache);
            }, 50);
        });
    });
});